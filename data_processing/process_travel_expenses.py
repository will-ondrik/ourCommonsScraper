...
def travel_claim_insert(start_date, end_date, transportation_cost, acccomodation_cost, meals_and_incidentals_cost, regular_points_used, special_points_used, usa_points_used, total_cost):
    return f"INSERT INTO TravelClaim (expenseId, memberId, startDate, endDate, transportationCosts, accommodationCosts, mealsAndIncidentalsCosts, regularPoints, specialPoints, usaPoints,totalCost) VALUES (@expenseId, @mpId, '{start_date}', '{end_date}', {transportation_cost}, {acccomodation_cost}, {meals_and_incidentals_cost}, {regular_points_used}, {special_points_used}, {usa_points_used}, {total_cost});"
...
def travel_insert(date, departure, destination, purpose):
    return f"INSERT INTO Travel (travellerId, claimId, date, departure, destination, purpose) VALUES (@travellerId, @claimId, '{date}', '{departure}', '{destination}', '{purpose}');"
    ...
    for data in travel_data:
        ...
        if not data['details']:
            ...
            for nested_data in data['details']:
                print('Nested travel data', nested_data)
